...
enum offsets : std::int32_t {
	world = 0xE9EA3F8,
	bonematrix = 0x1323A9C,
	staticfindobject = 0xD74AC0,
	processevent = 0x4879648
};

enum gender : std::uint8_t {
	male,
	female
};
...
public:
	...
	fort_item_tier get_tier() {
		return read<fort_item_tier>(std::uintptr_t(this) + 0x74);
	}

	ftext display_name() {
		return read<ftext>(std::uintptr_t(this) + 0x90);
	}
};
...
public:
	//int32_t GetMagazineAmmoCount(); // Function FortniteGame.FortWeapon.GetMagazineAmmoCount
	//int32_t GetBulletsPerClip(); // Function FortniteGame.FortWeapon.GetBulletsPerClip
...
public:
	bool already_searched() {
		return read<container_struct_0x102a>(std::uintptr_t(this) + 0x102a).bAlreadySearched;
	}
};
...
public:
	item_definition* get_item_definition() {
		return read<item_definition*>(std::uintptr_t(this) + 0x318 + 0x18);
	}
};
...
struct pawn_highlight {
	float Priority_28_E2E1B5344846E187B9C11B863A7F0698; // 0x00(0x04)
	flinearcolor Inner_21_4CC2801147EA190DE16F59B34F36853E; // 0x04(0x10)
...
public:
	...
	//void AddHighlightFromSource(struct UObject* Source, struct FPawnHighlight Highlight, bool OnlyUpdateIfCurrent);
	void add_highlight_from_source(uobject* source, pawn_highlight highlight, bool only_update_if_current) {
		struct {
			uobject* source;
			pawn_highlight highlight;
			bool only_update_if_current;
		} params = { source, highlight, only_update_if_current };

		static uobject* function;
		if (!function) function = uobject::static_find_object(nullptr, nullptr, L"PlayerPawn_Athena_C.AddHighlightFromSource", false);
		this->process_event(function, &params);
	}
	...
	gender get_gender() {
		...
		static uobject* function;
		if (!function) function = uobject::static_find_object(nullptr, nullptr, L"FortPlayerPawn.GetGender", false);
		this->process_event(function, &params);
		if (params.is_female)
	...
	//void SetFirstPersonCamera(bool bNewUseFirstPersonCamera); // Function FortniteGame.FortPlayerPawn.SetFirstPersonCamera
};

class aplayercontroller : public uobject {
public:
	float input_pitch_scale() {
		return read<float>(std::uintptr_t(this) + 0x52c);
	}

	float input_yaw_scale() {
		return read<float>(std::uintptr_t(this) + 0x528);
	}
...
namespace library {
	...
	public:
		fstring get_player_name_safe(actor* actor) {
			struct {
				uobject* actor;
				fstring return_value;
			} params = { actor };
			...
			static uobject* function;
			if (!function) function = uobject::static_find_object(nullptr, nullptr, L"FortKismetLibrary.GetPlayerNameSafe", false);
			this->process_event(function, &params);
			return params.return_value;
		}
	};
...
namespace defines {
	...
	static uworld* get_world() {
		return read<uworld*>(game + offsets::world);
	}
