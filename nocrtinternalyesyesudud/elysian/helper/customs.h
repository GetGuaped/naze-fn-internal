...
namespace elysian
{
	namespace customs
	{
		...
		auto wcsicmp_insensitive(wchar_t* cs, wchar_t* ct) -> int
		{
			auto len = wcslen(cs);
			if (len != wcslen(ct))
				return false;
		...
		auto in_rect(double x, double y, double radius, double coord_x, double coord_y) -> bool
		{
			return coord_x >= x - radius && coord_x <= x + radius &&
				coord_y >= y - radius && coord_y <= y + radius;
		}

		auto in_circle(double x, double y, double radius, double coord_x, double coord_y) -> bool
		{
		...
		auto get_module(wchar_t* name) -> std::uintptr_t
		{
